---
layout: default
title: Matemáticas para Ingeniería de Materiales
---

# Matemáticas para Ingeniería de Materiales

## Programa

- [Programa del curso](programa)

## Requisitos

- [Instalación y uso de Matlab](matlab)
- [Instalación y uso de LaTex](latex/)

## Actividades de clase

1. Modelos teóricos
   1. [Modelo matemático](modelo_teorico/modelo_matematico)
   1. [Modelos constitutivos elásticos](modelo_teorico/modelo_constitutivo_elastico)
   1. [Modelos no lineales elementales](modelo_teorico/modelos_no_lineales)
   1. Modelos de mezclas y materiales compuestos
   1. Modelos fenomenológicos en desgaste y adsorción

1. Modelado y simulación numérica
   1. Formulación de modelos a partir de balances físicos
   1. Escalamiento y simplificación de ecuaciones
   1. Ecuaciones diferenciales ordinarias con significado físico
   1. Discretización básica y simulación unidimensional
   1. Verificación e interpretación de resultados numéricos

1. Optimización y análisis multiobjetivo
   1. Formulación de problemas de optimización con restricciones
   1. Optimización aplicada al diseño de materiales
   1. Análisis multiobjetivo y compromisos de diseño
   1. Integración de modelos y optimización en casos reales

## Proyecto final

